...
function Dashboard() {
  ...
  var manualSell = async function(symbol, isReal) {
    ...
    try {
      var res = await fetch('/api/positions/close', {
        ...
        body: JSON.stringify({ symbol: symbol, is_real: isReal ? 1 : 0 })
      });
  ...
  var simOpenPositions = openPositions.filter(function(p) { return !p.is_real || p.is_real === 0; });
    ...
    <div className="dashboard">
      ...
      {realTradingEnabled && realOpenPositions.length > 0 && (
        <div className="table-container">
          <table>
            <thead>
              <tr><th>Sembol</th><th>Giris</th><th>Anlik</th><th>PnL%</th><th>Stop</th><th>AI</th><th>Acilis</th><th>Islem</th></tr>
            </thead>
            <tbody>
              {realOpenPositions.map(function(pos, i) {
                return (
                  <tr key={i} className={(pos.pnl_percent||0)>=0?'row-profit':'row-loss'}>
                    ...
                    <td>
                      <button onClick={function(){manualSell(pos.symbol,1);}} style={{
      ...
      {realTradingEnabled && realOpenPositions.length === 0 && (
        <div className="table-container">
          <table>
            ...
            <tbody><tr><td colSpan="8" style={{textAlign:'center',color:'#64748b',padding:30}}>Gercek alim aktif - Henuz acik pozisyon yok</td></tr></tbody>
          </table>
        </div>
      )}

      {/* SIMULASYON POZISYONLARI */}
                ...
                {simOpenPositions.slice(0,10).map(function(pos, i) {
                  return (
                    <tr key={i} className={(pos.pnl_percent||0)>=0?'row-profit':'row-loss'}>
                      ...
                      <td style={{fontSize:11,color:'#94a3b8'}}>{formatTime(pos.opened_at)}</td>
                      <td>
                        <button onClick={function(){manualSell(pos.symbol,0);}} style={{
                          padding:'6px 14px', background:'#6c5ce7', border:'none', borderRadius:6,
                          color:'#fff', fontSize:12, fontWeight:600, cursor:'pointer'
                        }}>SAT</button>
                      </td>
                    </tr>
