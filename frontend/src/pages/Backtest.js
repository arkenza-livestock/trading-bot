        ...
        <div>
          ...
          {loading && (
            <div style={{ background:'#0d1a2d', border:'1px solid #1e3a5f', borderRadius:8, padding:60, textAlign:'center' }}>
              ...
              <div style={{ fontSize:12, color:'#718096' }}>Tüm coinler aynı zaman çizgisinde test ediliyor</div>
              <div style={{ fontSize:12, color:'#4a5568', marginTop:8 }}>Max pozisyon, zarar koruması ve BTC filtresi aktif</div>
            </div>
          )}
            ...
            <div>
              ...
              <div className="card" style={{ marginBottom:16, padding:'10px 16px' }}>
                <div style={{ fontSize:12, color:'#718096', display:'flex', gap:16, flexWrap:'wrap' }}>
                  ...
                  <span>📊 Max Pos: <b style={{ color:'#e2e8f0' }}>{result.params.maxPositions}</b></span>
                </div>
              </div>
              ...
              <div className="card">
                ...
                <div style={{ overflowX:'auto' }}>
                  <table style={{ width:'100%', borderCollapse:'collapse' }}>
                    <thead>
                      <tr style={{ borderBottom:'1px solid #1e2736' }}>
                        {['Coin','Giriş','Çıkış','Giriş$','Çıkış$','Sebep','Skor','4H','1H','PnL'].map(h => (
                    ...
                    <tbody>
                      {result.trades.map((t, i) => (
                        <tr key={i} style={{ borderBottom:'1px solid #0d1117',
                          ...
                          <td style={{ padding:'8px 12px', fontSize:12, color:'#e2e8f0' }}>{parseFloat(t.entryPrice).toFixed(4)}</td>
                          <td style={{ padding:'8px 12px', fontSize:12, color:'#e2e8f0' }}>{parseFloat(t.exitPrice).toFixed(4)}</td>
                          ...
                          <td style={{ padding:'8px 12px', fontSize:12, color:'#f6ad55' }}>{t.score}</td>
                          <td style={{ padding:'8px 12px', fontSize:11, color:'#a0aec0' }}>{t.trend4H||'-'}</td>
                          <td style={{ padding:'8px 12px', fontSize:11, color:'#a0aec0' }}>{t.trend1H||'-'}</td>
