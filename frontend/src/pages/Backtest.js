  ...
  var Row = function(props) {
    return (
      <div style={{
        ...
        padding: '13px 0', borderBottom: '1px solid #111827'
      }}>
  ...
  return (
    <div style={{maxWidth: 520, padding: '32px 20px', margin: '0 auto'}}>
      ...
      <p style={{color: '#64748b', marginBottom: 30, fontSize: 13}}>
        🟢 LONG & 🔴 SHORT — Tum coinler ayni zaman cizgisi icinde test edilir
      </p>
      ...
      <div style={{background:'#0d1321', border:'1px solid #1a2540', borderRadius:14, padding:'20px 22px', marginBottom:16}}>
        ...
        <Row label="Test Suresi">
          <span style={{display:'flex', alignItems:'center', gap:6}}>
            <input name="days" type="number" value={params.days} onChange={handleChange} style={{width:60, background:'#0a0e17', border:'1px solid #334155', borderRadius:6, color:'#e2e8f0', padding:'6px 8px', fontSize:13, textAlign:'center'}} />
            <span style={{color:'#64748b', fontSize:13}}>Gun</span>
          </span>
        </Row>
        <Row label="AI Guven Esigi">
          <input name="machineConfidenceMin" type="number" step="0.05" value={params.machineConfidenceMin} onChange={handleChange} style={{width:70, background:'#0a0e17', border:'1px solid #334155', borderRadius:6, color:'#e2e8f0', padding:'6px 8px', fontSize:13, textAlign:'center'}} />
        </Row>
      </div>
      ...
      <button onClick={runBacktest} disabled={loading} style={{
        marginTop:8, padding:'14px 0', fontSize:15, fontWeight:600, width:'100%',
        background: loading ? '#1a1a2e' : '#1e293b', border:'1px solid #334155',
        borderRadius:10, color: loading ? '#64748b' : '#e2e8f0',
        cursor: loading ? 'not-allowed' : 'pointer'
      }}>
        {loading ? 'Calisiyor...' : '🚀 Backtest Calistir (LONG & SHORT)'}
      </button>
        ...
        <div style={{marginTop:25}}>
          ...
          <div className="card-grid" style={{gridTemplateColumns: 'repeat(2, 1fr)'}}>
            <div className="card" style={{borderLeft:'3px solid #22c55e'}}>
              <div className="card-label">🟢 LONG Islem</div>
              <div className="card-value" style={{fontSize:20}}>{results.summary ? results.summary.longCount : 0}</div>
              <div style={{fontSize:12, color:'#22c55e', marginTop:4}}>Başari: %{results.summary ? results.summary.longWinRate : 0}</div>
            </div>
            <div className="card" style={{borderLeft:'3px solid #ef4444'}}>
              <div className="card-label">🔴 SHORT Islem</div>
              <div className="card-value" style={{fontSize:20}}>{results.summary ? results.summary.shortCount : 0}</div>
              <div style={{fontSize:12, color:'#ef4444', marginTop:4}}>Başari: %{results.summary ? results.summary.shortWinRate : 0}</div>
            </div>
          </div>
          ...
          {results.trades && results.trades.length > 0 && (
            <div style={{marginTop:15}}>
              ...
              <div className="table-container">
                <table>
                  <thead>
                    <tr><th>Sembol</th><th>Yon</th><th>Giris</th><th>Cikis</th><th>PnL%</th><th>PnL</th><th>Neden</th></tr>
                  </thead>
                  <tbody>
                    {results.trades.slice(-20).reverse().map(function(trade, i) {
                      ...
                      return (
                        <tr key={i} className={trade.netPnl >= 0 ? 'row-profit' : 'row-loss'}>
                          ...
                          <td><span className={'badge ' + (isLong ? 'badge-buy' : 'badge-sell')}>{trade.side || 'LONG'}</span></td>
                          <td>{trade.entryPrice ? trade.entryPrice.toFixed(4) : '-'}</td>
                          <td>{trade.exitPrice ? trade.exitPrice.toFixed(4) : '-'}</td>
                          <td style={{color: trade.netPnl >= 0 ? '#22c55e' : '#ef4444', fontWeight: 600}}>%{trade.netPnlPct ? trade.netPnlPct.toFixed(2) : '0'}</td>
                          <td style={{color: trade.netPnl >= 0 ? '#22c55e' : '#ef4444'}}>{trade.netPnl ? trade.netPnl.toFixed(4) : '0'}</td>
