...
const COINS = ['BTCUSDT','ETHUSDT','BNBUSDT','SOLUSDT','XRPUSDT','ADAUSDT','DOGEUSDT','AVAXUSDT','DOTUSDT','LINKUSDT','MATICUSDT','LTCUSDT'];
...
export default function Backtest({ api }) {
  ...
  const run = async () => {
    ...
    setResult(null);
    try {
      const res = await axios.post(`${api}/api/backtest`, params);
      setResult(res.data);
    } catch (err) {
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: 20 }}>
        ...
        <div>
          ...
          {loading && (
            <div style={{ background: '#0d1a2d', border: '1px solid #1e3a5f', borderRadius: 8, padding: 40, textAlign: 'center', color: '#60a5fa' }}>
              <div style={{ fontSize: 32, marginBottom: 12 }}>⏳</div>
              <div style={{ fontSize: 15, fontWeight: 600 }}>Backtest çalışıyor...</div>
              <div style={{ fontSize: 12, color: '#718096', marginTop: 8 }}>
                Binance'den veri çekiliyor ve simüle ediliyor
              </div>
            </div>
          )}
          ...
          {result && !loading && (
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 20 }}>
                <StatCard
                  label="Ort. Kazanç"
                  value={`+%${result.summary.avgWin}`}
                  color="#68d391"
                />
                <StatCard
                  label="Ort. Kayıp"
                  value={`%${result.summary.avgLoss}`}
                  color="#fc8181"
                />
                <StatCard
                  label="En İyi İşlem"
                  value={`+%${result.summary.bestTrade}`}
                  color="#68d391"
                />
                <StatCard
                  label="En Kötü İşlem"
                  value={`%${result.summary.worstTrade}`}
                  color="#fc8181"
                />
              </div>
              ...
              <div className="card" style={{ marginBottom: 20, padding: '12px 16px' }}>
                <div style={{ fontSize: 12, color: '#718096', display: 'flex', gap: 20, flexWrap: 'wrap' }}>
                  <span>📊 <b style={{ color: '#e2e8f0' }}>{result.params.symbol}</b></span>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead>
                      <tr style={{ borderBottom: '1px solid #1e2736' }}>
                        ...
                        <th style={{ padding: '8px 12px', textAlign: 'right', fontSize: 12, color: '#718096' }}>Giriş Fiyat</th>
                        <th style={{ padding: '8px 12px', textAlign: 'right', fontSize: 12, color: '#718096' }}>Çıkış Fiyat</th>
                    ...
                    <tbody>
                      {result.trades.map((t, i) => (
                        <tr key={i} style={{ borderBottom: '1px solid #0d1117', background: t.netPnl >= 0 ? 'rgba(13,40,24,0.3)' : 'rgba(45,17,17,0.3)' }}>
                          ...
                          <td style={{ padding: '8px 12px', textAlign: 'right', fontWeight: 700, fontSize: 13, color: t.netPnl >= 0 ? '#68d391' : '#fc8181' }}>
