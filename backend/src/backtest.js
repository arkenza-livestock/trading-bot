...
class BacktestEngine {
  ...
  async run(params = {}) {
    ...
    console.log(`Ana mumlar: ${allCandles.length} | 1H mumlar: ${all1HCandles.length}`);
    ...
    for (let i = 50; i < allCandles.length; i++) {
      ...
      const candleTime= parseInt(allCandles[i][0]); // Bu mumun açılış zamanı (ms)

      // Bu muma karşılık gelen 1H mumları bul
      ...
      // 1H trend konfirmasyonu — doğru 1H mumlarıyla
      ...
      if      (trend1H.trend === 'YUKARI' && trend1H.guclu) finalScore = Math.min(100, finalScore + 20);
      else if (trend1H.trend === 'YUKARI')                   finalScore = Math.min(100, finalScore + 12);
      else if (trend1H.trend === 'HAFIF_YUKARI')             finalScore = Math.min(100, finalScore + 5);
      else if (trend1H.trend === 'HAFIF_ASAGI')              finalScore = Math.max(0,   finalScore - 15);
      ...
      if      (trend1H.trend === 'YUKARI' && trend1H.guclu) multiplier *= 1.3;
      else if (trend1H.trend === 'YUKARI')                   multiplier *= 1.2;
      else if (trend1H.trend === 'YATAY')                    multiplier *= 0.75;
      else if (trend1H.trend === 'HAFIF_ASAGI')              multiplier *= 0.5;
