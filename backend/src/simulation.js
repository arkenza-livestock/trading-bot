...
class SimulationEngine {
  ...
  openPosition(signal, settings={}) {
    try {
      ...
      const amount   = parseFloat((baseAmt).toFixed(2));
      if ((wallet?.balance||0) < amount) return null;

      const side     = signal.signal_type==='ALIM' ? 'LONG' : 'SHORT';
      const price    = signal.price || signal.fiyat;
      const quantity = amount / price;
      const guc      = signal.score >= 80 ? 'GUCLU' : signal.score >= 60 ? 'NORMAL' : 'ZAYIF';
      ...
      console.log(`[SIM] ✅ ${side} AÇILDI: ${signal.symbol} @ ${price} | ${amount} USDT | Güç:${guc}`);
