...
class TechnicalAnalysis {
  ...
  static calculateMACD(closes, fast = 12, slow = 26, signal = 9) {
    ...
    const prevHist = Math.abs(pm - ps);
    const curHist  = Math.abs(cm - cs);
    return {
  ...
  static calculateIchimoku(highs, lows, closes) {
    ...
    return {
      ...
      chikouBear:  price < (closes[len-26] || closes[0]),
      tkBull:  tenkan > kijun,
      tkBear:  tenkan < kijun,
      kumoBull: spanA > spanB,
      kumoBear: spanA < spanB,
      price
    };
  }
  ...
  static calculateMomentum(closes, volumes, highs, lows) {
    ...
    let ay=0, ak=0;
    for (let i=len-1; i>=Math.max(1,len-5); i--) { if (closes[i]>closes[i-1]) ay++; else break; }
    for (let i=len-1; i>=Math.max(1,len-5); i--) { if (closes[i]<closes[i-1]) ak++; else break; }
    const sonMum=closes[len-1]-closes[len-2], sonRange=highs[len-1]-lows[len-1];
    const govde = sonRange>0 ? Math.abs(sonMum)/sonRange : 0;
    ...
    let puan=0; const desc=[];
    if      (roc1>1.0)  puan+=30; else if (roc1>0.5)  puan+=20; else if (roc1>0.2)  puan+=10;
    else if (roc1>0)    puan+=5;  else if (roc1<-1.0) puan-=30; else if (roc1<-0.5) puan-=20;
    else if (roc1<-0.2) puan-=10; else puan-=5;
    if (roc1>0.3)  desc.push(`ROC:+${roc1.toFixed(2)}%`);
    if (roc1<-0.3) desc.push(`ROC:${roc1.toFixed(2)}%`);
    if      (ivme>0.3)  { puan+=20; desc.push('İvme↑'); }
    else if (ivme<-0.3) { puan-=20; desc.push('İvme↓'); }
    if      (ay>=4) { puan+=25; desc.push(`${ay}×yeşil`); }
    else if (ay===3) { puan+=15; }
    else if (ay===2) { puan+=8; }
    if (ak>=3) { puan-=25; desc.push(`${ak}×kırmızı`); }
    if (govde>0.7&&sonMum>0) { puan+=15; desc.push('Güçlü yeşil'); }
    else if (govde>0.7&&sonMum<0) { puan-=15; desc.push('Güçlü kırmızı'); }
    ...
    else if (volRoc>50&&roc1<0) { puan-=10; desc.push('Vol+Fiyat↓'); }
