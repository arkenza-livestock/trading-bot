...
class TechnicalAnalysis {
  ...
  static calculateMACD(closes, fast = 12, slow = 26, signal = 9) {
    ...
    const k_fast = 2 / (fast + 1), k_slow = 2 / (slow + 1), k_signal = 2 / (signal + 1);
    ...
    if (macdLine.length < signal + 1) return { macd: 0, signal: 0, histogram: 0, crossover: false, crossunder: false, bullish: false, bearish: false, histGrowing: false };
    let sig = macdLine.slice(0, signal).reduce((a,b)=>a+b,0)/signal;
    const sigLine = [];
    for (let i = signal; i < macdLine.length; i++) {
      sig = macdLine[i] * k_signal + sig * (1 - k_signal);
      sigLine.push(sig);
    }
    const curM = macdLine[macdLine.length-1], curS = sigLine[sigLine.length-1];
    const preM = macdLine[macdLine.length-2], preS = sigLine[sigLine.length-2];
    return {
      macd: parseFloat(curM.toFixed(8)), signal: parseFloat(curS.toFixed(8)),
      histogram: parseFloat((curM-curS).toFixed(8)),
      crossover:  preM <= preS && curM > curS,
      crossunder: preM >= preS && curM < curS,
      bullish: curM > curS, bearish: curM < curS,
      histGrowing: Math.abs(curM-curS) > Math.abs(preM-preS)
    };
  }
  ...
  static calculateIchimoku(highs, lows, closes) {
    ...
    return {
      tenkan, kijun, spanA, spanB, kumoUpper, kumoLower,
      aboveCloud: price > kumoUpper,
      belowCloud: price < kumoLower,
      insideCloud: price >= kumoLower && price <= kumoUpper,
      chikouBull: price > (closes[len-26] || closes[0]),
      tkBull: tenkan > kijun,
  ...
  static calculateTrendLine(closes, lows, lookback = 30) {
    if (lows.length < lookback) return { trendLine: lows[lows.length-1], nearLine: false, slope: 0, ascending: false };
    ...
    return {
      ...
      nearLine: price >= trendLine*(1-margin) && price <= trendLine*(1+margin),
  ...
  static calculateVolume(closes, volumes) {
    ...
    if (len < 20) return { puan: -100, desc: '', gecerli: false, oran: 0, alimOran: 50, spike: false };
    ...
    if (avg20 === 0) return { puan: -100, desc: '', gecerli: false, oran: 0, alimOran: 50, spike: false };
    ...
    return { puan, desc: desc.join(' '), oran:oran20, alimOran, vol5Trend, gecerli:true, spike:oran20>=1.5 };
  }

  static calculateMomentum(closes, volumes, highs, lows) {
    const len = closes.length;
    if (len < 6) return { puan: 0, desc: '', roc1: 0, ema5: 0, ema10: 0, ema20: 0 };
    ...
    for (let i=len-1; i>=Math.max(1,len-5); i--) { if (closes[i]>closes[i-1]) ardisikYesil++; else break; }
    for (let i=len-1; i>=Math.max(1,len-5); i--) { if (closes[i]<closes[i-1]) ardisikKirmizi++; else break; }
    const sonMum=closes[len-1]-closes[len-2], sonRange=highs[len-1]-lows[len-1];
    const govdeOran = sonRange>0 ? Math.abs(sonMum)/sonRange : 0;
    const ema5=this.calculateEMA(closes,5), ema10=this.calculateEMA(closes,10), ema20=this.calculateEMA(closes,20);
    ...
    if (roc1>0.3) desc.push(`ROC:+${roc1.toFixed(2)}%`);
    if (roc1<-0.3) desc.push(`ROC:${roc1.toFixed(2)}%`);
    if      (ivme>0.3)  { puan+=20; desc.push('İvme↑'); }
    else if (ivme<-0.3) { puan-=20; desc.push('İvme↓'); }
    if      (ardisikYesil>=4)  { puan+=25; desc.push(`${ardisikYesil}×yeşil`); }
    else if (ardisikYesil===3) { puan+=15; }
    else if (ardisikYesil===2) { puan+=8; }
    if (ardisikKirmizi>=3) { puan-=25; desc.push(`${ardisikKirmizi}×kırmızı`); }
    if (govdeOran>0.7&&sonMum>0) { puan+=15; desc.push('Güçlü yeşil'); }
    else if (govdeOran>0.7&&sonMum<0) { puan-=15; desc.push('Güçlü kırmızı'); }
    if (ema5>ema10&&ema10>ema20) { puan+=20; desc.push('EMA↑↑↑'); }
    else if (ema5>ema10) { puan+=10; }
    else if (ema5<ema10&&ema10<ema20) { puan-=20; desc.push('EMA↓↓↓'); }
    if (volRoc>50&&roc1>0) { puan+=10; desc.push('Vol+Fiyat↑'); }
    return { puan, desc:desc.join(' '), roc1, ema5, ema10, ema20 };
  }
  ...
  static calculateSR(closes, highs, lows, settings={}) {
    ...
    let puan=0; const desc=[];
    if      (pozisyon<8)  { puan=80;  desc.push(`🔥Destek`); }
    else if (pozisyon<20) { puan=60;  desc.push(`Destek yakını`); }
    else if (pozisyon<35) { puan=30; }
    else if (pozisyon<65) { puan=0; }
    else if (pozisyon<80) { puan=-30; }
    else if (pozisyon<92) { puan=-60; desc.push(`Direnç yakını`); }
    else                  { puan=-80; desc.push(`⚠️Direnç`); }
  ...
  static analyzeTF(candles) {
    ...
    const son10H=highs.slice(-10), son10L=lows.slice(-10);
    ...
    return { trend, puan, guclu, ema10, ema20, ema50, adx:adx.adx, diPlus:adx.diPlus, diMinus:adx.diMinus, rsi:rsi14, hhCount, hlCount };
  }
  ...
  static analyze(candles, ticker, settings={}) {
    ...
    else if (rsi<80)   { rsiPuan=-20; negative.push(`⚠️RSI(${rsi})`); }
    else               { rsiPuan=-60; negative.push(`🚫RSI(${rsi})`); }

    // MACD puan
    let macdPuan=0;
    if      (macd.crossover)              { macdPuan=100; positive.push('🚀MACD Crossover'); }
    else if (macd.bullish&&macd.histGrowing) { macdPuan=70; positive.push('📈MACD↑'); }
    else if (macd.bullish)                { macdPuan=40;  positive.push('📊MACD+'); }
    else if (macd.crossunder)             { macdPuan=-100; negative.push('💀MACD Crossunder'); }
    ...
    let ichimokuPuan=0;
    if (ichimoku) {
      if      (ichimoku.aboveCloud&&ichimoku.tkBull&&ichimoku.chikouBull) { ichimokuPuan=100; positive.push('☁️Ichimoku tam↑'); }
      else if (ichimoku.aboveCloud&&ichimoku.tkBull)  { ichimokuPuan=70; positive.push('☁️Bulut üstü+TK'); }
      ...
      else if (ichimoku.belowCloud&&!ichimoku.tkBull)  { ichimokuPuan=-60; negative.push('☁️Bulut altı↓'); }
      else if (ichimoku.belowCloud)                    { ichimokuPuan=-30; negative.push('☁️Bulut altı'); }
    }
    ...
    if (divergence.bearish)  { bonusPuan-=20; negative.push('🔀Bearish Div'); }
    if (trendLine.nearLine&&trendLine.ascending)  { bonusPuan+=15; positive.push('📏Trend desteği'); }
    else if (trendLine.nearLine)                   { bonusPuan+=8;  positive.push('📏Trend yakını'); }
    ...
    const score = Math.round(Math.max(-100, Math.min(100, agirlikliSkor)));
    const minScore = parseFloat(settings.min_score||10);

    // SHORT sinyali için ek kriterler
    const shortKriter = macd.crossunder || (macd.bearish && rsi > 65 && divergence.bearish);
    const longKriter  = macd.crossover  || (macd.bullish && rsi < 40 && divergence.bullish);

    let signal = 'BEKLE';
    if      (score >= minScore)   signal = 'ALIM';
    else if (score <= -minScore && shortKriter) signal = 'SATIS';
    else if (score <= -20)        signal = 'SATIS';

    const risk = score >= 70 ? 'DUSUK' : score >= 40 ? 'ORTA' : 'YUKSEK';
