...
class TechnicalAnalysis {
  ...
  // ── İCHİMOKU LONG — 5 alt koşul, 3/5 geçmeli ────────────
  ...
  // ── İCHİMOKU SHORT — 5 alt koşul, 3/5 geçmeli ───────────
  ...
  static analyze(candles, ticker, settings={}) {
    ...
    // Ichimoku kontrol
    const ichimokuL = this.ichimokuLongKontrol(ichimoku, rsi);
    const ichimokuS = this.ichimokuShortKontrol(ichimoku, rsi);
    ...
    const longK = {
      ...
      k5_ichimoku: ichimokuL.gecti,   // Ichimoku 3/5 tutmalı
    ...
    const shortK = {
      ...
      k5_ichimoku: ichimokuS.gecti,   // Ichimoku 3/5 tutmalı
      k6_momentum: ema5 < ema10 && roc1 < 0
    };
    ...
    const risk = isLong||isShort ? (ichimokuL.guclu||ichimokuS.guclu?'DUSUK':'ORTA') : 'YUKSEK';
    ...
    if (isLong) {
      positive.push(`✅ 6/6 LONG`);
      positive.push(`📊 RSI(${rsi})<52`);
      positive.push(macd.crossover?'🚀 MACD Cross↑':'📈 MACD+');
      positive.push(`💧 Hacim ${hacimOran.toFixed(1)}x`);
      positive.push(`📍 SR:%${pozisyon.toFixed(0)}`);
      positive.push(`☁️ Ichimoku ${ichimokuL.puan}/5${ichimokuL.guclu?' 💪':''}`);
      positive.push(`📈 EMA↑ +${roc1.toFixed(2)}%`);
      if (divergence.bullish) positive.push('🔀 Bullish Div');
    } else if (isShort) {
      negative.push(`⚠️ 6/6 SHORT`);
      negative.push(`📊 RSI(${rsi})>50`);
      negative.push(macd.crossunder?'💀 MACD Cross↓':'📉 MACD-');
      negative.push(`💧 Hacim ${hacimOran.toFixed(1)}x`);
      negative.push(`📍 SR:%${pozisyon.toFixed(0)}`);
      negative.push(`☁️ Ichimoku ${ichimokuS.puan}/5${ichimokuS.guclu?' 💪':''}`);
      ...
      if (longTutulan>=shortTutulan) {
        ...
        negative.push(`Long ${longTutulan}/6 Ich:${ichimokuL.puan}/5 eksik:${eksik.join(',')}`);
      } else {
        const eksik=Object.entries(shortK).filter(([,v])=>!v).map(([k])=>k);
        negative.push(`Short ${shortTutulan}/6 Ich:${ichimokuS.puan}/5 eksik:${eksik.join(',')}`);
      }
    }
